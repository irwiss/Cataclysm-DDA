...
task_reason veh_interact::cant_do( char mode )
{
    ...
    switch( mode ) {
        ...
        case 'o':
            ...
            if( cpart >= 0 && veh->can_unmount( veh->part( cpart ) ).success() ) {
                part_free = true;
            }
...
bool veh_interact::can_remove_part( int idx, const Character &you )
{
    ...
    const ret_val<void> status = veh->can_unmount( *sel_vehicle_part );
    if( !status.success() ) {
        //~ %1$s represents the internal color name which shouldn't be translated, %2$s is pre-translated reason
        nmsg += string_format( _( "> %1$s%2$s</color>" ), status_color( false ), status.str() ) + "\n";
        ok = false;
    }
...
void veh_interact::move_cursor( const point &d, int dstart_at )
{
    ...
    parts_here.clear();
    if( cpart >= 0 ) {
        for( const vehicle_part &vp : veh->parts_at_mount( veh->part( cpart ).mount ) ) {
            const int idx = veh->index_of_part( &vp );
            parts_here.emplace_back( idx );
            if( vp.is_repairable() || vp.is_broken() ) {
                need_repair.push_back( parts_here.size() - 1 );
            }
        }
    }
    ...
    switch( static_cast<char>( you.activity.index ) ) {
        ...
        case 'o': {
            ...
            if( veh->part_count_real() <= 1 ) {
                ...
                veh->part_removal_cleanup();
                if( veh->parts_at_mount( mount ).empty() ) {
